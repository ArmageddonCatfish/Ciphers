...
public class CypherOptimized {
	...
	// Fills the array with incrementing numbers starting at 33 up to 126
	public static void preformatKey(int[] key) {
			for (int element = 0; element < key.length; element++) {
				key[element] = element + NUMBER_OF_TRASH_CHARACTERS + 1;
			}
	}
	...
	public static void generateNewKey(int[] key) {
		...
		for (int element = 0; element < ARRAY_LENGTH; element++) {
			...
			int indexForSwap = r.nextInt(ARRAY_LENGTH);
